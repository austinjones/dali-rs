...
impl<'a> CanvasGate<'a> {
    pub(crate) fn new() -> CanvasGate<'a> {
        CanvasGate { layers: Vec::new() }
    }

    pub fn layer<F>(&mut self, colormap: &'a mut ColormapHandle, callback: F)
    where
    ...
    pub(crate) fn layers_mut(&mut self) -> impl Iterator<Item = &mut LayerGate<'a>> {
